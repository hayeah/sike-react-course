...
为了确保 `draw` 经常有足够的时间运行，我们可以使用 `requestAnimationFrame` 来调度 `draw` 在与屏幕刷新的相同间隔（bad translation）。时间线看起来像这样：
...
你可以使用 TweenMax 来驱动（bad translation） CSS 属性。三个最重要的方法是 `to`, `from` 和 `fromTo`。我们会使用一个叫 `#box` 的居中的元素来演示这些方法。

+ `TweenMax.to(object,duration,options)` - 从样式表 CSS 值驱动（bad translation）到你的值。
...
TweenMax.to("#box",2,{
  css: {
    // 在同时驱动（bad translation）多个 CSS 属性
    left: "200px",
    opacity: 0,
  },
});
...
对于用动画把元素移走的场景(bad translation)，这非常有用：

<video src="TweenMaxTo.mp4" autoplay controls loop></video>

[Codepen Demo](http://codepen.io/hayeah/pen/xwOmEj)

+ `TweenMax.from(object,duration,options)` - 从你的值驱动（bad translation）到 CSS 值。
...
对于把元素用动画移入（bad translation）的场景，这很有用：

<video src="TweenMaxFrom.mp4" autoplay controls loop></video>

[Codepen Demo](http://codepen.io/hayeah/full/NGreMb/)

+ `TweenMax.fromTo(object,duration,optionsFrom,optionsTo)` - 从你的初始值驱动（bad translation）到 你的最终值。
...
3. CPU 把点阵以文理形式上传到 GPU（bad translation）
4. 给 GPU 发送指令来处理点阵。平移/缩放/旋转，修改透明度，等等。
...
TweenMax 动画（bad translation）使用 `transform3d` 属性来启用 GPU 加速。使用 rendering 监控器，你可以看到“green rectangles”没了，不再有浏览器重绘了。此外，启用“show composited layer border”，你会看到元素有一个橙色边框，意为它是个 3D 图层，被 GPU 加速：
...
# 舞动机器人(bad translation)

TweenMax 从一个初始点到一个结束点执行动画（bad translation）。为了构建更复杂的动画，你可以使用 [TimeLineMax](https://greensock.com/docs/#/HTML5/GSAP/TimelineMax/) 来从 A 到 B 到 C 到 D 进行动画(bad translation)。
...
```js
// 让 the timeline 动画不停运行（bad translation）
var t = new TimelineMax({yoyo: true, repeat: -1});
```
...
# 高级 - Slider Control & 动画滚动效果（bad translation）

这部分是可选的。我们要实现两个额外的特性：

1. 更新 slider control 反映当前滚动的位置。
2. 当用户点击 slider control 跳转到一部分时，平滑地滚动到那里。
...
### 练习：滚动事件发生时更新 slider control

不论何时当窗口滚动时你应该使用 `window.scrollY` 来算出窗口正在展示的是哪一部分。

有四部分。你应该确保这些部分的 id 和 slider control 的 `href` 属性链接保持一致：
...
### 练习：滚动动画（bad translation）

当你点击 slider control 的链接时，浏览器立即跳转到被链接 `href` 属性定位的部分。现在我们想要使用 GreenSock 来平滑滚动到被定为到的部分。

[ScrollToPlugin](https://greensock.com/ScrollToPlugin) 提供了额外的功能来实现滚动动画（bad translation）。
...
通过使用被优化的动画引擎，JavaScript 动画能像 CSS 动画一样快。选择适合你项目任何一个更适合你项目的（bad translation）。

在 ReactNative 中，JavaScript 动画是你唯一的选择。很多我们已经谈到的性能技巧也适用于 ReactNative。最重要的是：

+ 避免引起布局。
+ 使用 GPU 可以加速的属性进行动画（bad translation）
